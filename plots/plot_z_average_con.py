...
if __name__ == "__main__":
    ...
    #############################################
        ...
        for species_idx, species in enumerate(specii):
            ...
            if species == 'delta':
                ax.plot(z, conc_mean[species]/por_mean, label="{}".format(r'$\delta$'), marker=marker, color=color)
            else:
                ax.plot(z, conc_mean[species]/por_mean, label="{}".format(species), marker=marker, color=color)
        
        ax.set_xlabel("Z")
        ax.set_ylabel("Avg. concentration")
        legend = plt.legend(fancybox=True, shadow=True, loc='upper right')
    #plt.show()
    #plt.tight_layout()
