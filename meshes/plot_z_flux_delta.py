'''
#command: python3 plot/plot_z_flux_delta.py --con /njord2/paimans/std_adv/2D/concentration/ -D 0.01 0.0025 0.001 --it_max 8
#command: python3 plot/plot_z_flux_delta.py --con /njord2/paimans/std_adv/bead_pack/bead_pack_bigger/concentration/ --L 0.15 -D 0.015 0.0015 0.00015 --it 6
'''
import dolfin as df
    ...
    for Pe_idx, Pe__ in enumerate(Pe):
        ...
        for it_idx, it in enumerate(it_values):
            ...
            for species in ['delta']:
                conc[species] = data[species]["conc"]
                Jz[species] = -data["uz"] * conc[species] - data[species]["J_diff"]
            Jz_mean['delta'] = Jz[species].mean(axis=(1, 2))

        #############################################
            z = Lz - zgrid #Lz - np.linspace(0., Lz, len(conc_mean[specii[0]]))

            ax.plot(z, Jz_mean['delta'], label='Iteration {}'.format(it),color=color, marker=marker, alpha=0.85)
            ax.set_title('Pe = {}'.format(int(Pe__))) 
            
            ax.set_xlabel("Z")
            ax.set_ylabel(r'$\mathrm{J_{\delta}}$')
            if Pe__ == 10:
                legend = plt.legend()
            plt.tight_layout()
        fig.savefig(args.con + 'plots/flux/flux_{}_Pe{}_itmax{}_different_it.pdf'.format(species,Pe__, it), dpi=300)
