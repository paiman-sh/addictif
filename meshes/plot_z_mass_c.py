'''
#command: python3 plot/plot_z_mass_c.py --con /njord2/paimans/std_adv/2D/concentration/ -D 0.01 0.0025 0.001 --it_max 8
'''
import dolfin as df
...
def parse_args():
    parser = argparse.ArgumentParser(description="mass of c")
...
if __name__ == "__main__":
    ...
    for Pe_idx, Pe__ in enumerate(Pe):
        ...
        #############################################
            ...
            ax.plot(z, conc_mean['c']*por_mean, label='Iteration {}'.format(it),color=color, marker=marker, alpha=0.85)
            #ax.set_title(r'$\alpha = {:.4f}$'.format(D_[Pe_idx])) 
            ax.set_title('Pe = {}'.format(int(Pe__))) 
            
            ax.set_xlabel("Z")
            ax.set_ylabel(r'$\mathrm{M_{c}}$')
           # legend = plt.legend(fancybox=True, shadow=True)
            plt.tight_layout()
        fig.savefig(args.con + 'plots/mass_c/mass_{}_Pe{}_itmax{}_different_it.pdf'.format(species,Pe__, it), dpi=300)
