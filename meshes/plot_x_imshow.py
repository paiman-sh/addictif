...
if __name__ == "__main__":
    ...
    for ix, x in enumerate(xgrid):
        for species in ['delta', 'c']:
            ...
            pcm = ax.pcolormesh(Y, Z, np.ma.masked_where(ma[:, :,ix], conc[species][:, :,ix]), cmap=cmap, shading="nearest")
            if args.arrows:
                ax.streamplot(Y, Z, np.ma.masked_where(ma[:, :,ix], data["uy"][:, :,ix]), data["uz"][:, :,ix], color='r', density=1.0)
            ax.set_aspect("equal")
            ax.set_xlabel("Y".format(species), fontsize=19)
            ax.set_ylabel("Z".format(species), fontsize=19)
            ...
            ax.set_title("Distance from the inlet: X = {:.2f}".format(xgrid[ix]), fontsize=20)
            plt.savefig(args.con + 'plots/imshow/Pe{}/video_{}_Pe={}_it{}_x_{}.png'.format(Pe__, species, Pe__, it, ix))
            plt.close()
